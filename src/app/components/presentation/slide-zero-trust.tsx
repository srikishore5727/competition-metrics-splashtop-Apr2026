...
// Secure Remote Access — Oct 2025 → Apr 2026
const TOTAL_KEYWORDS_INIT = {
  splashtop:   [19, 18, 17, 18, 13, 13, 16],
  teamviewer:  [10,  9, 10,  9,  7,  4,  5],
  anydesk:     [ 5,  3,  4,  4,  3,  2,  2],
  beyondtrust: [10,  9,  9,  8,  6,  8,  9],
  gotomypc:    [ 5,  4,  4,  3,  3,  2,  2],
};
const PAGE_ONE_INIT = {
  splashtop:   [16, 18, 16, 18, 12, 12, 15],
  teamviewer:  [ 5,  6,  5,  5,  5,  4,  5],
  anydesk:     [ 1,  1,  1,  1,  1,  1,  1],
  beyondtrust: [ 9,  8,  8,  7,  6,  7,  7],
  gotomypc:    [ 2,  1,  0,  0,  0,  1,  1],
};
const TRAFFIC_INIT = {
  splashtop:   [235, 318, 314, 374, 330, 426, 483],
  teamviewer:  [ 60, 123,  54,  67,  43,  70,  92],
  anydesk:     [  6,  23,  24,  24,  26,  25,  25],
  beyondtrust: [711, 790, 859, 827, 221, 500, 529],
  gotomypc:    [ 19,  14,   4,  16,  14,  22,  22],
};
...
export function SlideZeroTrust({ onNavigateHome }: Props) {
  ...
  const getInsights = () => {
    if (activeTab === 'keywords') return (
      <InsightsSection>
        <InsightCard icon={TrendingDown} type="error" title="Splashtop" content={<>From Oct 2025 to Apr 2026, keywords declined <strong>19 → 16 (−15.8%)</strong>, an opportunity to rebuild coverage.</>} />
        <InsightCard icon={TrendingDown} type="error" title="Competitions" content={<>TeamViewer declined <strong>10 → 5 (−50%)</strong>, while BeyondTrust declined <strong>10 → 9 (−10%)</strong> in Apr 2026.</>} />
      </InsightsSection>
    );
    if (activeTab === 'page-one') return (
      <InsightsSection>
        <InsightCard icon={TrendingDown} type="error" title="Splashtop" content={<>From Oct 2025 to Apr 2026, page-1 keywords declined <strong>16 → 15 (−6.3%)</strong>, visibility drop to address.</>} />
        <InsightCard icon={TrendingDown} type="error" title="Competitions" content={<>TeamViewer remained at <strong>5</strong>, while BeyondTrust declined <strong>9 → 7 (−22%)</strong> in Apr 2026.</>} />
      </InsightsSection>
    );
    return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop" content={<>From Oct 2025 to Apr 2026, traffic grew <strong>235 → 483 (+105.5%)</strong>, strong growth despite keyword decline.</>} />
        <InsightCard icon={TrendingDown} type="error" title="Competitions" content={<>BeyondTrust remained strong <strong>711 → 529 (−25.6%)</strong>, still leading despite decline in Apr 2026.</>} />
      </InsightsSection>
    );
  };
  ...
  return (
    <SlideContainer slideNumber={12} onNavigateHome={onNavigateHome} source="Ahrefs">
      <SlideHeader title="Secure Remote Access" subtitle="(Oct 2025 - Apr 2026)" />
      {!isEditing ? (
        ...
        <div className="flex-1 overflow-auto">
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <h3 className="text-sm font-bold text-gray-900 mb-4">Edit Secure Remote Access Data (Oct 2025 – Apr 2026)</h3>
