...
const REFERRING_DOMAINS_INIT: Record<string, number[]> = {
  splashtop:   [ 15946,  16955,  17078,  17107,  17310,  17851,  18038],
  teamviewer:  [123670, 122588, 119354, 117016, 116775, 118221, 118292],
  anydesk:     [ 40731,  40316,  39093,  38476,  38556,  39014,  38950],
  beyondtrust: [ 15880,  16786,  16330,  15835,  15881,  16027,  16062],
  gotomypc:    [  4581,   4450,   4501,   4380,   4232,   4205,   4191],
};

// ── Backlinks ────────────────────────────────────────────────────────────────
const BACKLINKS_INIT: Record<string, number[]> = {
  splashtop:   [ 1843951,  1385376,  1173208,  1154055,  1123979,  1018559,   864630],
  teamviewer:  [18841453, 18312611, 17975354, 18053149, 17932864, 17781816, 17036080],
  anydesk:     [ 8985522,  9999336, 10570441, 11060227, 10935451, 10403413,  9713870],
  beyondtrust: [  792939,   804609,   788689,   786337,   788440,   792394,   396918],
  gotomypc:    [   89053,    88209,    85758,    84712,    79873,    76063,    69388],
};
...
export function SlideBacklinks({ onNavigateHome }: { onNavigateHome?: () => void }) {
  ...
  const getInsights = () => {
    if (activeTab === 'referringDomains') return (
      <InsightsSection>
        <InsightCard icon={TrendingUp}   type="success" title="Splashtop"    content={<>From Oct 2025 to Apr 2026, referring domains grew <strong>15.9K → 18.0K (+13.1%)</strong>, consistent authority growth.</>} />
        <InsightCard icon={TrendingDown} type="error"   title="Competitions" content={<>TeamViewer declined <strong>123.7K → 118.3K (−4.3%)</strong>, while AnyDesk slightly declined <strong>40.7K → 39.0K (−4.4%)</strong>.</>} />
      </InsightsSection>
    );
    return (
      <InsightsSection>
        <InsightCard icon={TrendingDown} type="error"   title="Splashtop"    content={<>From Oct 2025 to Apr 2026, backlinks declined <strong>1.84M → 0.86M (−53.1%)</strong>, indicating link profile consolidation.</>} />
        <InsightCard icon={TrendingDown} type="error"   title="Competitions" content={<>TeamViewer declined <strong>18.8M → 17.0M (−9.6%)</strong>, while AnyDesk increased <strong>8.9M → 9.7M (+8.1%)</strong>.</>} />
      </InsightsSection>
    );
  };
  ...
  return (
    <SlideContainer slideNumber={18} onNavigateHome={onNavigateHome}>
      <SlideHeader
        ...
        subtitle="(Oct 2025 - Apr 2026) • Location: WW"
      />
      ...
      {!isEditing ? (
        ...
        <div className="flex-1 overflow-auto">
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <h3 className="text-sm font-bold text-gray-900 mb-4">Edit Backlink Data (Oct 2025 – Apr 2026)</h3>
