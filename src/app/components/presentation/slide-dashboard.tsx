...
// Remote Connection — Oct 2025 → Apr 2026
const TOTAL_KEYWORDS_INIT = {
  splashtop:   [10, 10, 11, 11, 11, 13, 13],
  teamviewer:  [22, 22, 22, 21, 20, 20, 20],
  anydesk:     [20, 19, 17, 19, 17, 15, 15],
  beyondtrust: [ 4,  2,  3,  5,  5,  5,  5],
  gotomypc:    [13, 13, 10, 13, 12, 12, 12],
};
const PAGE_ONE_INIT = {
  splashtop:   [ 7,  7,  8,  8,  9, 11, 11],
  teamviewer:  [21, 21, 21, 21, 20, 20, 20],
  anydesk:     [18, 16, 16, 18, 16, 14, 14],
  beyondtrust: [ 2,  1,  2,  4,  4,  5,  5],
  gotomypc:    [ 8, 13,  9,  8,  6,  7,  7],
};
const TRAFFIC_INIT = {
  splashtop:   [ 26,  36,  60,  49,  63, 111, 114],
  teamviewer:  [139, 275, 270, 200, 228, 325, 335],
  anydesk:     [127,  87, 187, 142, 261, 568, 591],
  beyondtrust: [  1,   1,   3,   7,   7,  10,  10],
  gotomypc:    [ 18,  46,  37,  53,  42,  18,  19],
};
...
export function SlideDashboard({ onNavigateHome }: Props) {
  ...
  const getInsights = () => {
    if (activeTab === 'keywords') return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop" content={<>From Oct 2025 to Apr 2026, keywords grew <strong>10 → 13 (+30%)</strong>, while TeamViewer leads steadily at <strong>20</strong>.</>} />
        <InsightCard icon={TrendingDown} type="error" title="Competitions" content={<>TeamViewer declined <strong>22 → 20 (−9.1%)</strong>, while AnyDesk declined <strong>20 → 15 (−25%)</strong> in Apr 2026.</>} />
      </InsightsSection>
    );
    if (activeTab === 'page-one') return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop" content={<>From Oct 2025 to Apr 2026, page-1 keywords grew <strong>7 → 11 (+57%)</strong>, strong top-SERP improvement.</>} />
        <InsightCard icon={TrendingDown} type="error" title="Competitions" content={<>TeamViewer declined <strong>21 → 20 (−4.8%)</strong>, while AnyDesk declined <strong>18 → 14 (−22%)</strong> in Apr 2026.</>} />
      </InsightsSection>
    );
    return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop" content={<>From Oct 2025 to Apr 2026, traffic surged <strong>26 → 114 (+338%)</strong>, the strongest percentage gain.</>} />
        <InsightCard icon={TrendingUp} type="success" title="Competitions" content={<>TeamViewer grew <strong>139 → 335 (+141%)</strong>, while AnyDesk surged <strong>127 → 591 (+365%)</strong> in Apr 2026.</>} />
      </InsightsSection>
    );
  };
  ...
  return (
    <SlideContainer slideNumber={11} onNavigateHome={onNavigateHome} source="Ahrefs">
      <SlideHeader title="Remote Connection" subtitle="(Oct 2025 - Apr 2026)" />
      {!isEditing ? (
        ...
        <div className="flex-1 overflow-auto">
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <h3 className="text-sm font-bold text-gray-900 mb-4">Edit Remote Connection Data (Oct 2025 – Apr 2026)</h3>
