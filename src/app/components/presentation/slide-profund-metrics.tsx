...
const MONTHS_DISPLAY = ['Oct 2025', 'Nov 2025', 'Dec 2025', 'Jan 2026', 'Feb 2026', 'Mar 2026', 'Apr 2026'];
const MONTH_KEYS     = ['2025-10',  '2025-11',  '2025-12',  '2026-01',  '2026-02',  '2026-03',  '2026-04'];
...
const AI_KEYWORDS_INIT = {
  splashtop:   [1117, 1193, 1483, 1764, 1972, 2403, 2409],
  teamviewer:  [1480, 1490, 1580, 1787, 1880, 2715, 3383],
  anydesk:     [ 422,  370,  403,  519,  559,  725,  958],
  beyondtrust: [ 515,  516,  603,  744,  747,  901, 1236],
  gotomypc:    [  36,   38,   39,   36,   27,   35,   44],
};

// ── Branded AI keywords ───────────────────────────────────────────────────────
const BRANDED_INIT = {
  splashtop:   [  85,  101,  109,  142,  149,  196,  253],
  teamviewer:  [ 685,  678,  655,  701,  694,  730,  981],
  anydesk:     [ 227,  208,  194,  231,  241,  244,  318],
  beyondtrust: [ 108,   89,   89,  123,  114,  154,  201],
  gotomypc:    [   8,    7,    6,   14,   10,    6,    8],
};

// ── Non-Branded AI keywords ───────────────────────────────────────────────────
const NON_BRANDED_INIT = {
  splashtop:   [1032, 1092, 1374, 1622, 1823, 2207, 2156],
  teamviewer:  [ 795,  812,  965, 1054, 1186, 1985, 2402],
  anydesk:     [ 195,  162,  209,  288,  318,  481,  640],
  beyondtrust: [ 407,  427,  514,  621,  633,  747, 1035],
  gotomypc:    [  28,   31,   33,   22,   17,   29,   36],
};
...
export function SlideProfundMetrics({ onNavigateHome }: { onNavigateHome?: () => void }) {
  ...
  const latestIdx = 6; // Apr 2026

  const getInsights = () => {
    if (activeTab === 'aiKeywords') return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop"    content={<>From Oct 2025 to Apr 2026, AI keywords grew <strong>1,117 → 2,409 (+115.7%)</strong>, strong expansion in AI visibility.</>} />
        <InsightCard icon={TrendingUp} type="success" title="Competitions" content={<>TeamViewer grew <strong>1,480 → 3,383 (+128.6%)</strong>, while AnyDesk increased <strong>422 → 958 (+127.0%)</strong>.</>} />
      </InsightsSection>
    );
    if (activeTab === 'branded') return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop"    content={<>From Oct 2025 to Apr 2026, branded AI keywords grew <strong>85 → 253 (+197.6%)</strong>, strongest branded growth in the set.</>} />
        <InsightCard icon={TrendingUp} type="success" title="Competitions" content={<>TeamViewer leads branded at <strong>685 → 981 (+43.2%)</strong>, while AnyDesk grew <strong>227 → 318 (+40.1%)</strong>.</>} />
      </InsightsSection>
    );
    return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop"    content={<>From Oct 2025 to Apr 2026, non-branded AI keywords grew <strong>1,032 → 2,156 (+108.9%)</strong>, discovery-led growth.</>} />
        <InsightCard icon={TrendingUp} type="success" title="Competitions" content={<>TeamViewer grew <strong>795 → 2,402 (+202.1%)</strong>, while BeyondTrust increased <strong>407 → 1,035 (+154.3%)</strong>.</>} />
      </InsightsSection>
    );
  };
  ...
  const editMonths = ['Oct','Nov','Dec','Jan','Feb','Mar','Apr'];

  return (
    <SlideContainer slideNumber={14} onNavigateHome={onNavigateHome} source="SEMrush">
      <SlideHeader title="AI Overview Keywords" subtitle="(Oct 2025 - Apr 2026)" />
      ...
      {!isEditing ? (
        ...
        <div className="flex-1 overflow-auto">
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <h3 className="text-sm font-bold text-gray-900 mb-4">Edit AI Overview Keywords Data (Oct 2025 – Apr 2026)</h3>
