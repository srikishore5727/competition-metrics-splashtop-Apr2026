...
// Remote Support — Oct 2025 → Apr 2026
const TOTAL_KEYWORDS_INIT = {
  splashtop:   [ 80,  78,  78,  86,  81,  81,  81],
  teamviewer:  [ 93,  91,  86,  84,  83,  78,  78],
  anydesk:     [ 83,  79,  81,  83,  77,  66,  66],
  beyondtrust: [104, 100,  73,  74,  73,  73,  73],
  gotomypc:    [ 21,  14,  19,  19,  14,   6,   6],
};
const PAGE_ONE_INIT = {
  splashtop:   [64, 74, 72, 80, 74, 78, 78],
  teamviewer:  [83, 85, 78, 74, 72, 73, 73],
  anydesk:     [75, 75, 75, 78, 71, 60, 60],
  beyondtrust: [84, 90, 64, 66, 61, 65, 65],
  gotomypc:    [ 5,  5,  8,  7,  6,  2,  2],
};
const TRAFFIC_INIT = {
  splashtop:   [ 506,  655,  561,  880,  816,  690,  702],
  teamviewer:  [1206, 1030, 1037, 1369, 1752, 1651, 1696],
  anydesk:     [1406, 1311, 1525, 1543, 1682, 1255, 1259],
  beyondtrust: [2416, 2492,  502,  604,  871,  889,  897],
  gotomypc:    [  43,   23,   37,   36,   21,    4,    4],
};
...
export function SlideOTSecurity({ onNavigateHome }: Props) {
  ...
  const getInsights = () => {
    if (activeTab === 'keywords') return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop" content={<>From Oct 2025 to Apr 2026, keywords remained stable <strong>80 → 81 (+1.3%)</strong>, consistent category coverage.</>} />
        <InsightCard icon={TrendingDown} type="error" title="Competitions" content={<>TeamViewer declined <strong>93 → 78 (−16.1%)</strong>, while AnyDesk dropped <strong>83 → 66 (−20.5%)</strong> in Apr 2026.</>} />
      </InsightsSection>
    );
    if (activeTab === 'page-one') return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop" content={<>From Oct 2025 to Apr 2026, page-1 keywords grew <strong>64 → 78 (+21.9%)</strong>, improving SERP visibility.</>} />
        <InsightCard icon={TrendingDown} type="error" title="Competitions" content={<>TeamViewer declined <strong>83 → 73 (−12%)</strong>, while AnyDesk declined <strong>75 → 60 (−20%)</strong> in Apr 2026.</>} />
      </InsightsSection>
    );
    return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop" content={<>From Oct 2025 to Apr 2026, traffic increased <strong>506 → 702 (+38.7%)</strong>, showing steady upward growth.</>} />
        <InsightCard icon={TrendingUp} type="success" title="Competitions" content={<>TeamViewer grew <strong>1,206 → 1,696 (+40.6%)</strong>, while AnyDesk declined <strong>1,406 → 1,259 (−10.5%)</strong>.</>} />
      </InsightsSection>
    );
  };
  ...
  return (
    <SlideContainer slideNumber={10} onNavigateHome={onNavigateHome} source="Ahrefs">
      <SlideHeader title="Remote Support" subtitle="(Oct 2025 - Apr 2026)" />
      {!isEditing ? (
        ...
        <div className="flex-1 overflow-auto">
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <h3 className="text-sm font-bold text-gray-900 mb-4">Edit Remote Support Data (Oct 2025 – Apr 2026)</h3>
