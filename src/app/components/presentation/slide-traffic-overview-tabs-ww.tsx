...
// ─────────────────────────────────────────────────────────────
// Months  Oct 2025 → Apr 2026  (7 data points)
// ─────────────────────────────────────────────────────────────
const WW_MONTHS = ['2025-10', '2025-11', '2025-12', '2026-01', '2026-02', '2026-03', '2026-04'];
...
const ORGANIC_TRAFFIC_WW: Record<string, number[]> = {
  splashtop:   [142000, 130000, 137000, 142000, 151000, 159000, 174000],
  teamviewer:  [500000, 478000, 522000, 551000, 504000, 523000, 514000],
  anydesk:     [416000, 407000, 418000, 427000, 426000, 441000, 439000],
  beyondtrust: [ 58000,  55000,  52000,  46000,  47000,  39000,  41000],
  gotomypc:    [ 48000,  47000,  65000,  64000,  64000,  65000,  64000],
};

const BRANDED_TRAFFIC_WW: Record<string, number[]> = {
  splashtop:   [103000,  82000,  93000,  93000,  94000,  90000,  92000],
  teamviewer:  [444000, 360000, 411000, 431000, 425000, 425000, 429000],
  anydesk:     [395000, 359000, 388000, 403000, 403000, 413000, 414000],
  beyondtrust: [ 17000,  19000,  16000,  14000,  16000,   8000,   9000],
  gotomypc:    [ 43000,  43000,  60000,  60000,  61000,  61000,  60000],
};

const NON_BRANDED_TRAFFIC_WW: Record<string, number[]> = {
  splashtop:   [ 39000,  48000,  45000,  49000,  57000,  69000,  81000],
  teamviewer:  [ 56000, 118000, 111000, 120000,  79000,  98000,  84000],
  anydesk:     [ 21000,  48000,  30000,  24000,  23000,  28000,  25000],
  beyondtrust: [ 41000,  36000,  35000,  31000,  31000,  31000,  32000],
  gotomypc:    [  5000,   4000,   4000,   4000,   3000,   4000,   3000],
};
...
export function SlideTrafficOverviewTabsWW({ onNavigateHome }: SlideTrafficOverviewTabsWWProps) {
      ...
      return (
        <InsightsSection>
          <InsightCard
            ...
            type="success"
            title="Splashtop"
            content={<>From Oct 2025 to Apr 2026, organic traffic grew <strong>142K → 174K (+22.5%)</strong>, steady and sustained growth.</>}
          />
          <InsightCard
            icon={TrendingUp}
            type="success"
            title="Competitions"
            content={<>TeamViewer grew <strong>500K → 514K (+2.8%)</strong> and AnyDesk increased <strong>416K → 439K (+5.5%)</strong>, moderate gains.</>}
          />
        </InsightsSection>
      );
      ...
      return (
        <InsightsSection>
          <InsightCard
            ...
            type="error"
            title="Splashtop"
            content={<>From Oct 2025 to Apr 2026, branded traffic shifted <strong>103K → 92K (−10.7%)</strong>, while overall grew.</>}
          />
          <InsightCard
            icon={TrendingDown}
            type="error"
            title="Competitions"
            content={<>TeamViewer declined <strong>444K → 429K (−3.4%)</strong>; AnyDesk increased <strong>395K → 414K (+4.8%)</strong>.</>}
          />
        </InsightsSection>
      );
    } else {
      return (
        <InsightsSection>
          <InsightCard
            ...
            type="success"
            title="Splashtop"
            content={<>From Oct 2025 to Apr 2026, non-branded traffic surged <strong>39K → 81K (+107.7%)</strong>, strongest discovery gain.</>}
          />
          <InsightCard
            icon={TrendingUp}
            type="success"
            title="Competitions"
            content={<>TeamViewer grew <strong>56K → 84K (+50%)</strong>; AnyDesk increased <strong>21K → 25K (+19%)</strong> in Apr 2026.</>}
          />
        </InsightsSection>
      );
    }
  ...
  const getPerformanceSummary = () => {
    const latestIdx = WW_MONTHS.length - 1; // Apr 2026
  ...
  return (
    <SlideContainer slideNumber={3} onNavigateHome={onNavigateHome}>
      <SlideHeader
        ...
        subtitle="(Oct 2025 - Apr 2026) · Worldwide"
      />
