...
const AI_TRAFFIC_INIT = {
  splashtop:   [ 4107,  6442,  9782, 10787, 12769, 15258, 13228],
  teamviewer:  [ 9332, 15600,  9363, 48299, 11044, 19248, 18752],
  anydesk:     [ 2058,  1872,  3719,  2899,  2580,  5943,  6581],
  beyondtrust: [ 5166,  2928,  5197,  2904,  4135,  5329,  7924],
  gotomypc:    [   96,   112,   221,    60,    41,   198,   113],
};

// ── Branded AI Traffic ────────────────────────────────────────────────────
const BRANDED_INIT = {
  splashtop:   [  882, 1049,  819,  766, 3745, 6049, 6494],
  teamviewer:  [ 4153, 3469, 3093, 3787, 4777, 7931, 8985],
  anydesk:     [ 1274, 1163, 2663, 1860, 1448, 2574, 3831],
  beyondtrust: [ 3670, 1405, 2860, 1270, 2095, 2191, 4210],
  gotomypc:    [   26,   26,   11,    9,    3,   15,   15],
};

// ── Non-Branded AI Traffic ────────────────────────────────────────────────
const NON_BRANDED_INIT = {
  splashtop:   [ 3225,  5393,  8963, 10021,  9024,  9209,  6734],
  teamviewer:  [ 5179, 12131,  6270, 44512,  6267, 11317,  9767],
  anydesk:     [  784,   709,  1056,  1039,  1132,  3369,  2750],
  beyondtrust: [ 1496,  1523,  2337,  1634,  2040,  3138,  3714],
  gotomypc:    [   70,    86,   210,    51,    38,   183,    98],
};
...
export function SlideCategoryPerformance({ onNavigateHome }: { onNavigateHome?: () => void }) {
  ...
  const getInsights = () => {
    if (activeTab === 'aiTraffic') return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop"    content={<>From Oct 2025 to Apr 2026, AI traffic grew <strong>4,107 → 13,228 (+222.1%)</strong>, strong acceleration in AI-driven discovery.</>} />
        <InsightCard icon={TrendingUp} type="success" title="Competitions" content={<>TeamViewer grew <strong>9,332 → 18,752 (+101.0%)</strong>, while AnyDesk increased <strong>2,058 → 6,581 (+219.8%)</strong>.</>} />
      </InsightsSection>
    );
    if (activeTab === 'branded') return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop"    content={<>From Oct 2025 to Apr 2026, branded traffic grew <strong>882 → 6,494 (+636.3%)</strong>, rising brand visibility in AI results.</>} />
        <InsightCard icon={TrendingUp} type="success" title="Competitions" content={<>TeamViewer increased <strong>4,153 → 8,985 (+116.3%)</strong>, while AnyDesk grew <strong>1,274 → 3,831 (+200.7%)</strong>.</>} />
      </InsightsSection>
    );
    return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop"    content={<>From Oct 2025 to Apr 2026, non-branded traffic grew <strong>3,225 → 6,734 (+108.8%)</strong>, indicating discovery-led AI growth.</>} />
        <InsightCard icon={TrendingUp} type="success" title="Competitions" content={<>TeamViewer grew <strong>5,179 → 9,767 (+88.6%)</strong>, while AnyDesk increased <strong>784 → 2,750 (+250.8%)</strong>.</>} />
      </InsightsSection>
    );
  };
  ...
  return (
    <SlideContainer slideNumber={15} onNavigateHome={onNavigateHome} source="SEMrush">
      <SlideHeader title="AI Overview Traffic" subtitle="(Oct 2025 - Apr 2026)" />
      ...
      {!isEditing ? (
        ...
        <div className="flex-1 overflow-auto">
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <h3 className="text-sm font-bold text-gray-900 mb-4">Edit AI Overview Traffic Data (Oct 2025 – Apr 2026)</h3>
