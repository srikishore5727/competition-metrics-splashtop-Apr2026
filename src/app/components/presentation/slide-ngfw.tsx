...
// Remote Access — Oct 2025 → Apr 2026
const TOTAL_KEYWORDS_INIT = {
  splashtop:   [399, 367, 371, 380, 361, 363, 363],
  teamviewer:  [402, 374, 371, 371, 350, 350, 349],
  anydesk:     [295, 259, 273, 269, 266, 216, 216],
  beyondtrust: [ 71,  58,  60,  63,  69,  71,  73],
  gotomypc:    [204, 178, 191, 200, 182, 150, 150],
};
const PAGE_ONE_INIT = {
  splashtop:   [297, 318, 317, 321, 302, 320, 320],
  teamviewer:  [336, 336, 339, 330, 318, 325, 324],
  anydesk:     [213, 225, 234, 223, 219, 188, 188],
  beyondtrust: [ 38,  41,  38,  36,  40,  49,  51],
  gotomypc:    [138, 135, 132, 133, 122, 110, 110],
};
const TRAFFIC_INIT = {
  splashtop:   [1555, 1971, 2638, 2823, 2728, 2656, 2676],
  teamviewer:  [3544, 5814, 3608, 3811, 3659, 4922, 4516],
  anydesk:     [1538, 1692, 2196, 2261, 2044, 2448, 2472],
  beyondtrust: [1687, 1728, 1521, 1212,  776,  985, 1039],
  gotomypc:    [ 735,  677,  794,  964,  644,  669,  670],
};
...
export function SlideNGFW({ onNavigateHome }: Props) {
  ...
  const getInsights = () => {
    if (activeTab === 'keywords') return (
      <InsightsSection>
        <InsightCard icon={TrendingDown} type="error" title="Splashtop" content={<>From Oct 2025 to Apr 2026, keywords declined <strong>399 → 363 (−9.0%)</strong>, remaining relatively stable overall.</>} />
        <InsightCard icon={TrendingDown} type="error" title="Competitions" content={<>TeamViewer declined <strong>402 → 349 (−13.2%)</strong>, while AnyDesk dropped <strong>295 → 216 (−26.8%)</strong>.</>} />
      </InsightsSection>
    );
    if (activeTab === 'page-one') return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop" content={<>From Oct 2025 to Apr 2026, page-1 keywords grew <strong>297 → 320 (+7.7%)</strong>, improving SERP visibility.</>} />
        <InsightCard icon={TrendingDown} type="error" title="Competitions" content={<>TeamViewer declined <strong>336 → 324 (−3.6%)</strong>, while AnyDesk dropped <strong>213 → 188 (−11.7%)</strong>.</>} />
      </InsightsSection>
    );
    return (
      <InsightsSection>
        <InsightCard icon={TrendingUp} type="success" title="Splashtop" content={<>From Oct 2025 to Apr 2026, traffic increased <strong>1,555 → 2,676 (+72.1%)</strong>, strong growth rate.</>} />
        <InsightCard icon={TrendingUp} type="success" title="Competitions" content={<>TeamViewer grew <strong>3,544 → 4,516 (+27.4%)</strong>, while AnyDesk increased <strong>1,538 → 2,472 (+60.7%)</strong>.</>} />
      </InsightsSection>
    );
  };
  ...
  return (
    <SlideContainer slideNumber={8} onNavigateHome={onNavigateHome} source="Ahrefs">
      <SlideHeader title="Remote Access" subtitle="(Oct 2025 - Apr 2026)" />
      {!isEditing ? (
        ...
        <div className="flex-1 overflow-auto">
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <h3 className="text-sm font-bold text-gray-900 mb-4">Edit Remote Access Data (Oct 2025 – Apr 2026)</h3>
